...
class CitationDataloader(object):
    ...
    def _load(self):
        ...
        if not self.random_split:
            ...
            np.random.seed(0)
            valuable_nodes = test_idx_range.tolist() + list(range(len(y))) + list(range(len(y), len(y)+500))
            print("Split train-val-test 0.5-0.3-0.2.")
            indices = np.random.permutation(valuable_nodes)
            n_train = int(len(indices) * 0.5)
            n_val = int(len(indices) * 0.3)
