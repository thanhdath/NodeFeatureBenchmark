    ...
    def __init__(self, datadir, random_split=False):
        ...
        if "cora" in datadir:
            if not random_split:
                ...
                self.train_mask = torch.BoolTensor(_sample_mask(range(140), self.labels.shape[0]))
                self.val_mask = torch.BoolTensor(_sample_mask(range(200, 500), self.labels.shape[0]))
                self.test_mask = torch.BoolTensor(_sample_mask(range(500, 1500), self.labels.shape[0]))
            else:
                ...
                self.train_mask = torch.BoolTensor(_sample_mask(indices[:n_train], self.labels.shape[0]))
                self.val_mask = torch.BoolTensor(_sample_mask(indices[n_train:n_train+n_val], self.labels.shape[0]))
                self.test_mask = torch.BoolTensor(_sample_mask(indices[n_train+n_val:], self.labels.shape[0]))
...
class DefaultInductiveDataloader():
    ...
    def _load(self):
        ...
        self.features = np.load('{}/{}_feats.npy'.format(self.datadir, self.mode), allow_pickle=True)
        # if self.mode == "train":
